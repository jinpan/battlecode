...
public class BreadthFirst {
	...
	public int height;
	public int width;
	public Direction[][] pathingData;
	public int[][] distanceData;//integer comparisons are apparently cheaper (2 vs 4 b)
	public int[][] mapData;
	public ArrayList<MapLocation> path = new ArrayList<MapLocation>();
	public Direction[] dirs = {Direction.NORTH,Direction.NORTH_EAST,Direction.EAST,Direction.SOUTH_EAST,Direction.SOUTH,Direction.SOUTH_WEST,Direction.WEST,Direction.NORTH_WEST};
	//public Direction[] dirs = {Direction.NORTH,Direction.EAST,Direction.SOUTH,Direction.WEST};
	public boolean shortestPathLocated;
	public RobotController rci;
	public Hashtable<MapLocation,Direction[][]> storedPathingData = new Hashtable<MapLocation,Direction[][]>();
	
	//pathTo(myLoc,enemy,1000000);
	
	public BreadthFirst(RobotController rci,int bigBoxSize){
		rc = rci;
	...
	public int getMapData(MapLocation m){
		return getMapData(m.x,m.y);
	}
	
	private void setMapData(int x, int y, int val){
		mapData[x+1][y+1] = val;
	}
	
	private void updateInternalMap(RobotController rc){//can take several rounds, but ultimately saves time
	...
	public ArrayList<MapLocation> pathTo(MapLocation start,MapLocation uncheckedGoal, int maxSearchDist) throws GameActionException {
	...
	public MapLocation trimGoal(MapLocation uncheckedGoal){
	...
	private ArrayList<MapLocation> getNewOutermost(ArrayList<MapLocation> outermost,MapLocation start,MapLocation goal){
		...
		for(MapLocation m:outermost){
			Proposal.generateProposals(m, distanceData[m.x][m.y],1, props, dirs, this);//all proposals are traversible
		}
...
//			}
	...
	private void listDirections(MapLocation start,MapLocation end){
	...
	public Direction getNextDirection(ArrayList<MapLocation> path, int bigBoxSize){
	...
	public void printDirectionArray(MapLocation start, MapLocation goal){
		System.out.println("Direction map:");
		System.out.println("start: "+start+", goal: "+goal);
		for(int x=0;x<this.pathingData[0].length;x++){
			for(int y=0;y<this.pathingData.length;y++){
				Direction d = this.pathingData[x][y];
				if(d==null){
					System.out.print("X");
				}else{
