...
public class SoldierPlayer extends BaseRobot {
	...
	protected void setup() throws GameActionException {
		int msg= this.myRC.readBroadcast(ACTION_BROADCAST_CHANNEL);
	...
	protected void attack_step() throws GameActionException {
		...
		if ((this.myRC.canSenseSquare(robotTask.targetLocation) 
				&& this.myRC.senseObjectAtLocation(robotTask.targetLocation) == null) 
				|| robotTask.targetLocation.equals(this.enemyHQLoc)) {
			return;
		} else {
			move_to_target(robotTask.targetLocation, false);
	...
	protected boolean respond_to_threat(boolean defending) throws GameActionException{
		...
		int maxHeur = 0;
		if(defending)
			maxHeur = ourLoc.distanceSquaredTo(robotTask.targetLocation);
	...
	protected int getHeuristic(RobotInfo ri, MapLocation orig){
		int health = (int)ri.health;
		int dist = orig.distanceSquaredTo(ri.location);
	...
	protected void defend_step() throws GameActionException {		
		if (this.myRC.isActive()){
			...
			MapLocation target = robotTask.targetLocation;
